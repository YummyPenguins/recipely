...
class RecipeDetailScreen extends Component {
  ...
  componentDidMount() {
    ...
    if (!this.state.ingredients) {
      fetch(
        `https://yummypenguin-recipely.herokuapp.com/api/recipes/${recipe_id}`
      )
        .then(res => res.json())
        .then(result =>
          this.setState({ ingredients: result.recipe.ingredients })
        );
    }
  }
      ...
      <ScrollView>
        ...
        >
          ...
          <View style={styles.buttonMargins}>
            <Button title="Directions" onPress={this.handlePressButtonAsync} />
            <Button
